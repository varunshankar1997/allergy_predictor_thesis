...
#NETMHCIIpath="/services/tools/netmhciipan/4.2a"
#NETMHCIIpan="netMHCIIpan"

logdir="$HM/mhciipan_extend-5_full/results/log"
cshdir="$HM/mhciipan_extend-5_full/results/csh"

mkdir -p $logdir
mkdir -p $cshdir

SEQ_folder="$HM/net_allergen_batch2"
ALLELELIST="$HM/7alleles.txt"
result_dir="$HM/mhciipan_extend-5_full/results"
...
do
    ...
    printf "#PBS -l nodes=1:ppn=1:thinnode,mem=16gb,walltime=8:59:00\n" >> $cscript 
    printf "#PBS -d $logdir\n" >> $cscript
...
#$NETMHCpan -a HLA-A02:01 -p $TEST > pred
